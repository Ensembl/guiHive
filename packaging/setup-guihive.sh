...
EHIVE_SOURCE='https://github.com/Ensembl/ensembl-hive.git'
GUIHIVE_SOURCE='https://github.com/Ensembl/guiHive.git'

umask 0002

if [ -d "$GUIHIVE_VERSIONS_DIR" ] && [ -d "$EHIVE_VERSIONS_DIR" ]
then
  rm -rf "$GUIHIVE_VERSIONS_DIR"
  rm -rf "$EHIVE_VERSIONS_DIR"
fi
...
safe_clone () {
  ...
  mkdir -p "$3"
  echo GIT_DIR="$2" GIT_WORK_TREE="$3" git checkout -qf "$1"
...
add_guihive_version () {
  echo "add_guihive_version: checkout $1: guiHive $2 and eHive $3"
...
link_guihive_version () {
  ...
  echo "link_guihive_version: checkout $1: guiHive =$2 and eHive $EHIVE_COMMIT"
...
then
  ...
  echo GIT_DIR="$GUIHIVE_CLONE_LOCATION" GIT_WORK_TREE="$DEPLOY_LOCATION" git checkout -qf 'server'
...
# Removed support for versions 57 to 79
